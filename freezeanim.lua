...
local FreezeModule = {
    ...
    createGUI = function(self)
        ...
        self:setupEvents()
        self.api:addToActive("freeze_gui", gui)
    end,
    ...
    toggleFreezeMode = function(self)
        self.isFreezeModeActive = not self.isFreezeModeActive
        self:applyState()
    end,

    getOriginalAnimations = function(self, Animate)
    ...
    applyState = function(self)
        ...
        if not character or not character:FindFirstChild("Animate") or not character:FindFirstChild("HumanoidRootPart") then
            self:connectToCharacter()
            return
        end
        ...
        self:getOriginalAnimations(Animate)
        
        if self.isFreezeModeActive then
            -- Set the character's root part to be anchored, which freezes all movement
            HumanoidRootPart.Anchored = true
            
            -- Stop all animations to give a truly "frozen" look
            ...
            self.toggleButton.Text = "Frozen"
            self.toggleButton.BackgroundColor3 = Color3.fromRGB(100, 255, 120)
        else
            -- Restore the original state, un-anchoring the character
            ...
            self.toggleButton.Text = "Toggle Freeze"
    ...
    connectToCharacter = function(self)
        ...
        self.characterConnection = LocalPlayer.CharacterAdded:Connect(function(character)
            -- Wait for the 'Animate' script and 'HumanoidRootPart' to be in the new character model
    ...
    onUnload = function(self)
        ...
        -- Restore original values when the script is unloaded
        self.isFreezeModeActive = false
        self:applyState()
        if self.gui then
            self.gui:Destroy()
            self.gui = nil
